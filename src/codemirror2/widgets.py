...
class CodeMirrorEditor(widgets.Textarea):
    ...
    def render(self, name, value, attrs=None, **kwargs):
        ...
        return rendered + mark_safe(render_to_string(self.script_template, context))

    @property
    def media(self):
        js_files = ["codemirror2/lib/codemirror.js"]
        js_files += ["codemirror2/addon/mode/overlay.js"]
        js_files += [(f"codemirror2/mode/{mode}/{mode}.js") for mode in self.modes]
        css_files = ["codemirror2/lib/codemirror.css"]
        css_files += [
            ("codemirror2/theme/%s.css" % theme)
        ...
        for mode in need_css:
            if mode in self.modes:
                css_files.append(f"codemirror2/mode/{mode}/{mode}.css")
